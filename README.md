...
This repo builds on Erwin de Kreuk's [FMD Framework](https://github.com/edkreuk/FMD_FRAMEWORK)
(MIT licensed) with two additions: a Gold layer (`gold.GoldEntity`, `ai.FeatureSet`, and
the loader/demo notebooks under `src/business_domain/`) and a deployment model where
**one notebook does everything**, triggered from Azure DevOps.

**Start with [DEPLOYMENT.md](DEPLOYMENT.md)** — the one-time manual bootstrap, then how
`azure-pipelines.yml` takes over from there. Everything below this point is upstream
FMD's own documentation, kept as background reference.
