...
# # Gold Loader Functions
# Generic dimension and fact loading utilities for the Gold layer of a schema-enabled Fabric lakehouse.
# `%run` this notebook from any `dim_*` / `fact_*` notebook, then call `load_dimension()` or `load_fact()`.
#
# Supports SCD Type 1 and Type 2 dimensions, several fact write patterns (overwrite, append, upsert,
# incremental, replace_partition), automatic surrogate-key generation, automatic business-key-to-surrogate-key
# mapping on fact tables, and an "Unknown" member row (sk = -1) on every dimension.
...
def write_dimension_type1(
    ...
    lakehouse_name: str,
    table_name: str,
    schema_name: str = GOLD_SCHEMA,
    table_prefix: str = DIM_TABLE_PREFIX,
    full_refresh: bool = False,
    recreate_table: bool = False,
    create_unknown_record: bool = True
) -> DataFrame:
    """Load a Slowly Changing Dimension Type 1 (overwrite changes) into the Gold schema."""
...
def write_dimension_type2(
    ...
    """Load a Slowly Changing Dimension Type 2 (track history) into the Gold schema."""
    table_name = table_name.lower()
    _ensure_schema(lakehouse_name, schema_name)
    full_table_name = _full_table_name(lakehouse_name, schema_name, table_prefix, table_name)
    table_exists = not recreate_table and spark.catalog.tableExists(full_table_name)
...
def load_dimension(
    ...
    """
    Load a dimension table into the Gold layer. Facade over SCD Type 1 / Type 2.

    Args:
        df: Source DataFrame. Business key column(s) should end in '_key'.
        lakehouse_name: Target Gold lakehouse (e.g. 'LH_GOLD_LAYER').
...
def load_fact(
    ...
    Args:
        ...
        lakehouse_name: Target Gold lakehouse.
        table_name: Table name without prefix (e.g. 'orderlines' -> gold.fact_orderlines).
        schema_name: Target schema (default 'gold').
